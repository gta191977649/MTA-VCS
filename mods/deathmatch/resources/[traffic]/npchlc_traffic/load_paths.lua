...
function loadPaths()
	...
	iprint(node_conns[1][30448])
	--[[
	for i=1,100 do 
		local x,y,z = node_x[i],node_y[i],node_z[i]
		createMarker(x,y,z)
		createBlip(x,y,z)
		-- create conns_1
		
		if node_conns[i] then
			for key,conn in pairs(node_conns[i]) do 
				local cx,cy,cz = node_x[key],node_y[key],node_z[key]
				createMarker(cx,cy,cz+2,"arrow",2,0,255,0,255)
				createBlip(cx,cy,cz,0,2,0,255,0,255)
			end
		end
		-- draw conn_n1,n2
		local n1 = conn_n1[i]
		local n2 = conn_n2[i]
		local nb = conn_n2[i]
		local cx,cy,cz = node_x[n1],node_y[n1],node_z[n1]
		createMarker(cx,cy,cz+2,"arrow",2,0,0,255,255)
		createBlip(cx,cy,cz,0,2,0,0,255,255)
		local cx,cy,cz = node_x[n2],node_y[n2],node_z[n2]
		createMarker(cx,cy,cz+2,"arrow",2,0,0,255,255)
		createBlip(cx,cy,cz,0,2,0,0,255,255)
		local cx,cy,cz = node_x[nb],node_y[nb],node_z[nb]
		createMarker(cx,cy,cz+2,"arrow",2,255,255,255,255)
		createBlip(cx,cy,cz,0,2,255,255,255,255)
	end

	iprint(square_cdens[3])
	]]
end
