...
//------------------------------------------------------------------------------------------
// MTACalcGTAPedSpecularLights
// - Calculate GTA dynamic lighting for peds (all 4 lights)
//------------------------------------------------------------------------------------------
float3 MTACalcGTAPedSpecularLights(float3 WorldNormal, float SpecPow )
{
    // Add all the 4 pointlights
    float DirectionFactor = 0;	
    ...
    if (gLight1Enable) {
    DirectionFactor = dot(gLight1Direction, -WorldNormal);
    TotalDiffuse += pow(DirectionFactor * 0.5 + 0.5, 1.5) * gLight1Diffuse.rgb;
    TotalSpecular += pow(max(DirectionFactor, 0.0), SpecPow + 1) * gLight1Diffuse.rgb;
                       }
    if (gLight2Enable) {
    DirectionFactor = dot(gLight2Direction, -WorldNormal);
    TotalDiffuse += pow(DirectionFactor * 0.5 + 0.5, 1.5) * gLight2Diffuse.rgb;
    TotalSpecular += pow(max(DirectionFactor, 0.0), SpecPow + 1) * gLight2Diffuse.rgb;	
                       }
    if (gLight3Enable) {
    DirectionFactor = dot(gLight3Direction, -WorldNormal);
    TotalDiffuse += pow(DirectionFactor * 0.5 + 0.5, 1.5) * gLight3Diffuse.rgb;
    TotalSpecular += pow(max(DirectionFactor, 0.0), SpecPow + 1) * gLight3Diffuse.rgb;	
                       }
    if (gLight4Enable) {
    DirectionFactor = dot(gLight4Direction, -WorldNormal);
    TotalDiffuse += pow(DirectionFactor * 0.5 + 0.5, 1.5) * gLight4Diffuse.rgb;
    TotalSpecular += pow(max(DirectionFactor, 0.0), SpecPow + 1) * gLight4Diffuse.rgb;	
                       }
    TotalDiffuse = saturate(TotalDiffuse); TotalSpecular = saturate(TotalSpecular); 
    float3 OutDiffuse = saturate(TotalDiffuse.rgb * TotalSpecular.rgb);
