...
PSInput VertexShaderFunction(VSInput VS)
{
    ...
    float4 Normal;
    if (gDeclNormal != 1) Normal = float4(0,0,0,0);
...
Pixel PixelShaderFunction(PSInput PS)
{
    ...
    if (PS.Normal.w == 0) 
    {
        VNormal = normalize(cross(ddx(PS.ViewPos),ddy(PS.ViewPos)));
        VNormal = mul(VNormal, (float3x3)gViewInverse);
    } 	
        else VNormal = PS.Normal.xyz;
   ...
   output.Normal = float4((Normal.xy * 0.5) + 0.5, Normal.z <0 ? 0.411 : 0.589, 1);
...
// Techniques
//------------------------------------------------------------------------------------------
technique RTinput_world_normal
{
    pass P0
    {
