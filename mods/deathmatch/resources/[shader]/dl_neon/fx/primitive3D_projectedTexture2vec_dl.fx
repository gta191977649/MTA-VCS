// 
// file: primitive3D_projectedTexture2vec_dl.fx
// version: v1.6
// author: Ren712
//
...
Pixel PixelShaderFunction(PSInput PS)
{
    ...
    // World, Color and Normal render targets set to 0
    output.World = 0;
    output.Color = 0;
    output.Normal = 0;
	
    // disregard calculations when depth value is close to 1 and beyound light radius
    if (bufferValue > 0.99999f) return output;
    if ((linearDepth - PS.ProjCoord.w) > 0) return output;
	
    // get world normal from normalRT
    float3 texNormal = tex2D(SamplerNormal, TexProj.xy).xyz;
    if (texNormal.z > 0.5999f) return output;
    ...
    float lum = saturate(finalColor.r + finalColor.g + finalColor.b);

    // World and Color render targets
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawPrimitive3DProjectedTexture2vec_dl
{
  pass P0
  {
...
technique dxDrawPrimitive3DProjectedTexture2vec_fallback
{
  pass P0
  {
    ZEnable = true;
