// 
// file: primitive3D_spotLight_normalGen.fx
// version: v1.6
// author: Ren712
//
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    {
        // get world normal from normal texture
...
int ChooseCullMode()
{
    if (sLightBillboard == false) 
    {
        if ((length(gViewInverse[3].xyz - sLightPosition)) < max(1, tan(sLightPhi)) * sLightAttenuation * 1.5) return 3;
        else return 2;
    }
    else return 2;
}
...
bool ChooseZEnable()
{
    if (sLightBillboard == false)
    {
        if (length(gViewInverse[3].xyz - sLightPosition) < max(1, tan(sLightPhi)) * 2 * sLightAttenuation) return false;
        else return true;
    }
    else return true;
}
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawPrimitive3DSpotLight_normalGen
{
  pass P0
  {
